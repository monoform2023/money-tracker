...
const firebaseConfig = {
    ...
    databaseURL: "https://money-tracker-d35c7-default-rtdb.firebaseio.com", // Добавьте это
    projectId: "money-tracker-d35c7",
    storageBucket: "money-tracker-d35c7.appspot.com",
...
function loadData() {
    ...
    transactionsRef.on('value', (snapshot) => {
        ...
        if (data) {
            history = Object.entries(data).map(([id, transaction]) => ({ id, ...transaction }));
            updateBalances();
            updateHistory();
        }
    });
...
function addTransaction() {
    ...
    transactionsRef.push(transaction)
        .then(() => {
            console.log("Транзакция добавлена в Firebase!");
        })
        .catch((error) => {
